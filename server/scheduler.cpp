...
static thread_local Scheduler* t_scheduler = nullptr;   //当前正在执行的协程调度器
static thread_local Fiber* t_fiber = nullptr;   //run主协程
...
void Scheduler::start() {
    ...
    // if(m_rootFiber) {
    //     m_rootFiber->call();
    //     SERVER_LOG_INFO(g_logger) << "call out";
    // }
}
...
void Scheduler::stop() {
    ...
    if(m_rootFiber && m_threadCount == 0 
        ...
        SERVER_LOG_INFO(g_logger) << this << " stoped";
        m_stopping = true;
    ...
    if(m_rootThread != -1) {        //如果use_caller 只能在创建的线程中stop  否则在其他线程中stop
